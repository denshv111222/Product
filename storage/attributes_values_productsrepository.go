...
func (atvalprodRep *Attributs_values_productsrepository) FilterAllAttributes_values_products(fil *models.Filter) ([]*models.Attributes_values_products, error) { //срань которую надо переделать(а можно и не переделывать)
	...
	query := fmt.Sprintf("Select count(*) FROM %s,%s %s", tableAttributes_values_products, where, sort)
	if err := atvalprodRep.storage.db.QueryRow(query).Scan(&fil.Pages.AllRecords); err != nil {
		return nil, err
	}
